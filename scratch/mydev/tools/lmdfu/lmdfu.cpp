                ...
                tLMDFUHandle *phHandle)
{
    ...
    for(iIntCount = 0; iIntCount < pucConfig[4]; iIntCount++)
    {
        ...
        else
        {
            ...
            {
                //
                // Yes - break out of the search loop.
                //
                break;
            }
            else
    ...
    //
    // Find the DFU functional descriptor.
    //
    pucDFU = FindDescriptor(USB_DESC_TYPE_DFU,
                            pucConfig + pucConfig[0],
                            ((unsigned long)(READ_SHORT(pucConfig + 2)) -
                             (unsigned long)pucConfig[0]));
    ...
    //
    // Make sure the device is idle before we do anything that may be illegal
    // in various error states.
    //
