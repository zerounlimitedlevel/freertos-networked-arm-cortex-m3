...
//
//*****************************************************************************
const unsigned short g_usApplicationVersion = 6075;
...
ShowVersion(void)
{
    ...
    printf("Copyright (c) 2009 Texas Instruments Incorporated\n\n");
}
...
main(int argc, char *argv[])
{
    ...
    //
    for (idx = 0; idx < iKeyBits / 32; idx++)
    {
        sscanf(pcKeyString, "%8X", &uKeyBuf[idx]);
        pcKeyString += 8;
    }
    ...
    {
        aes_setkey_enc(&ctx, (unsigned char*)uKeyBuf, iKeyBits);
        pcKeyString = "Encrypt";
    }
    else
    {
        aes_setkey_dec(&ctx, (unsigned char*)uKeyBuf, iKeyBits);
        pcKeyString = "Decrypt";
    }
